...
Основная публичная витрина HumanizerOS рассчитана на англоязычную аудиторию. Эта страница описывает тот же продукт для русскоязычного пользователя. Русские правила написаны отдельно от английских: language pack использует собственные конструкции, исключения и eval-сценарии.
...
Fact Guard сравнивает защищённые значения до и после. Он контролирует прямые цитаты, имена собственные, числа, проценты, цены, единицы, даты, время, URL, email, версии, UUID, commit-like хеши, идентификаторы и код. Для прямых цитат допускается замена straight/curly типографики кавычек без изменения текста внутри.
...
- Fact Guard с защитой прямых цитат и типизированных поверхностных фактов;
- text, JSON и SARIF 2.1.0;
- CLI и Python API;
- root `SKILL.md` для Agent Skills ecosystem;
- отдельные `humanizer-os-en` и `humanizer-os-ru` skills;
- 175 автоматических тестов и 59 двуязычных eval-сценариев.
